...
@section('title', 'Tạo Đợt Đánh Giá Mới')

@section('content')
<section class="mod-period-create bg-white rounded-2xl custom-box-shadow py-8">
  <div class="container max-w-4xl mx-auto px-4">    
    <div class="mb-8 flex justify-between items-center">
      <div>
        <h2 class="text-base mb-3">Tạo Đợt Đánh Giá Mới</h2>
    ...
    <div class="bg-white rounded-xl custom-box-shadow overflow-hidden mb-8 border border-gray-200">
      ...
      <div class="p-6">
        <form action="{{ route('periods.store') }}" method="POST">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div class="relative">
              ...
              <div class="mt-1 relative">
                ...
                <input type="date" id="end_date" name="end_date" value="{{ old('end_date', date('Y-m-d', strtotime('+7 days '))) }}" 
          ...
          <div class="mb-8">
            <label class="block text-base font-medium text-gray-700 mb-3">Trạng thái <span class="text-red-500">*</span></label>
            <div class="flex gap-6">
              @foreach($statuses as $status)
                @if($status->name == 'Mở' || $status->name == 'Đóng')
                  <div class="flex items-center">
                    <input type="radio" 
                          ...
                          value="{{ $status->id }}"
                          class="h-5 w-5 mr-2"
                          {{ old('status_id') == $status->id ? 'checked' : 
                            (old('status_id') === null && $status->name == 'Mở' ? 'checked' : '') }}
                          required>
                    
                    <label for="status_{{ $status->id }}" class="cursor-pointer {{ $status->name == 'Mở' ? 'text-green-600' : 'text-red-600' }}">
                      {{ $status->name }}
                    </label>
                  </div>
                @endif
