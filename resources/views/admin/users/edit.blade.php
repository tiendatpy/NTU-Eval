    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            ...
            <input type="text" name="full_name" id="full_name" value="{{ old('full_name', $user->full_name) }}" required
                class="w-full rounded-2xl border border-gray-300 bg-primary-050 px-4 py-3 focus:border-states-500 focus:outline-none">
        </div>
        ...
        <div>
            ...
            <input type="date" name="date_of_birth" id="date_of_birth" 
                value="{{ old('date_of_birth', $user->date_of_birth ? date('Y-m-d', strtotime($user->date_of_birth)) : '') }}" required
                class="w-full rounded-2xl border border-gray-300 bg-primary-050 px-4 py-3 focus:border-states-500 focus:outline-none">
        </div>
        ...
        <div>
            ...
            <input type="text" name="phone" id="phone" value="{{ old('phone', $user->phone) }}" required
                class="w-full rounded-2xl border border-gray-300 bg-primary-050 px-4 py-3 focus:border-states-500 focus:outline-none">
            <p class="text-xs text-gray-500 mt-1">Nhập đúng 10 chữ số, không kèm ký tự đặc biệt</p>
        </div>
